...
BASE_OUTPUT_PATH=r"C:\Users\isaac\PycharmProjects\document_localization\Recursive-CNNs\full_document_datasets\complete_documents\kosmos"

NUMBER_OF_3_CORNERS=500
NUMBER_OF_2_CORNERS=500
NUMBER_OF_1_CORNERS=500
...
def create_crop(sample,number_of_corners,padding):
    ...
    keep_points=rotated_keypoints[:number_of_corners]
    exclude_cordinate=rotated_keypoints[-1]

    # #%%
    cropped_image=crop_image_with_keypoints(rotated_image, keep_points, exclude_cordinate,padding)
    return cropped_image

for _ in range(NUMBER_OF_1_CORNERS):
    sample = loader_smart_doc_train[random.randint(0, len(loader_smart_doc_train) - 1)]
    try:
        cut_img=create_crop(sample,1,padding=200)
        image = Image.fromarray(cut_img)
        print("NUMBER_OF_1_CORNERS:",_)
    except:
        pass

    # Save the image as a JPG file
    image.save(os.path.join(BASE_OUTPUT_PATH,"1_corners",f'{_}.jpg'), format='JPEG')
#%%
for _ in range(NUMBER_OF_2_CORNERS):
    sample = loader_smart_doc_train[random.randint(0, len(loader_smart_doc_train) - 1)]
    try:
        cut_img=create_crop(sample,2,padding=50)
        image = Image.fromarray(cut_img)
        print("NUMBER_OF_2_CORNERS:",_)
    except:
        pass
    # Save the image as a JPEG file
    image.save(os.path.join(BASE_OUTPUT_PATH,"2_corners",f'{_}.jpg'), format='JPEG')
#%%
for _ in range(NUMBER_OF_3_CORNERS):
    sample = loader_smart_doc_train[random.randint(0, len(loader_smart_doc_train) - 1)]
    try:
        cut_img=create_crop(sample,3,padding=10)
        image = Image.fromarray(cut_img)
        print("NUMBER_OF_3_CORNERS:",_)
    except:
        pass
    # Save the image as a JPEG file
    image.save(os.path.join(BASE_OUTPUT_PATH,"3_corners",f'{_}.jpg'), format='JPEG')
