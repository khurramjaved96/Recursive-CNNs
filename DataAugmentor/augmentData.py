                ...
                with open(dir+"/"+image+ ".csv", 'r') as csvfile:
                    ...
                    for angle in range(0,271,90):
                        ...
                        for random_crop in range(0,1):
                            ...
                            # for a in range(0,4):
                            for a in range(0,4):
                                img_crop = cv2.circle(img_crop, tuple((gt_crop[a].astype(int))), 2, (255, 0, 0), 9)
                            # cv2.imwrite("asda.jpg", img)
                            # 0/0
                            cv2.imwrite(output_dir + "/" + dir +str(angle)+str(random_crop)+ image, img_crop)
                            spamwriter_1.writerow((dir +str(angle)+str(random_crop)+ image, gt_crop))
